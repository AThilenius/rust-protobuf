...
impl<'a> CodeWriter<'a> {
    ...
    pub fn with_no_error(f: impl FnOnce(&mut CodeWriter)) -> String {
        Self::with_impl::<Infallible, _>(|w| Ok(f(w))).unwrap_or_else(|e| match e {})
    }

    pub fn with<F>(f: F) -> anyhow::Result<String>
    where
    ...
    pub fn _write_text(&mut self, text: &str) {
    ...
    pub fn fn_block<F>(&mut self, vis: Visibility, sig: &str, cb: F)
    where
        F: FnOnce(&mut CodeWriter),
    {
        match vis {
